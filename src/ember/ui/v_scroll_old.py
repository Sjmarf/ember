...
class VScroll(Scroll):
    ...
    def _update_element_rect(self) -> None:
        if self._element:
            with log.size.indent:
                ...
                if self.can_scroll:
                    element_y = self.rect.y - self._scroll.val
        ...
        if event.type == pygame.MOUSEWHEEL and self.can_scroll:
            if self.rect.collidepoint(_c.mouse_pos):
                self._scroll.val = pygame.math.clamp(
                    self._scroll.val - event.y * self._style.scroll_speed,
                    -self.over_scroll[0],
    ...
    def scroll_to_show_position(
        ...
        if position - offset <= self.rect.y:
            position -= offset
            destination = self._scroll.val - (self.rect.y - position)
        elif position + size + offset > self.rect.bottom:
            position += offset
            destination = (
                    self._scroll.val - (self.rect.y - position) - self.rect.h + size
            )
        else:
            return
