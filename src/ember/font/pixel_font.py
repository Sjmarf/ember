...
class PixelFont(BaseFont):
    def __init__(
        ...
        self.sheet = pygame.image.load(
            os.path.join(filename, "font.png")
        ).convert_alpha()
    ...
    def _render_text(
        ...
        surf = pygame.Surface(
            (max(1, self.get_width_of(text)), self.line_height), pygame.SRCALPHA
        )
        x = -3
        for letter in text:
            letter = self.letters[letter]
            letter_surf = self.sheet.subsurface(
                (letter[0], 0, letter[1], self.line_height)
            )
            surf.blit(letter_surf, (x, 0))
            x += letter[1] - 5

        surf.fill(color, special_flags=pygame.BLEND_RGB_ADD)
        return surf
