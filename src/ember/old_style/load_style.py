...
from typing import Union, IO, Optional, Type, Sequence
import os
...
from .. import style
from ..style import state

import pygame

from ..size import FIT, FILL, Size, AbsoluteSize, FitSize, FillSize
...
def _decode_size(
    ...
    if isinstance(size, list):
        if len(size) == 3:
            if size[0] == "FIT":
                return FitSize(size[2], size[1])
            elif size[0] == "FILL":
                return FillSize(size[2], size[1])
            else:
                return AbsoluteSize(size[1])
        else:
            sizes = []
            for i in size:
