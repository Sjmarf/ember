...
class ClampedSize(ResizableSize):
    ...
    def __add__(self, other: int) -> "ClampedSize":
        return ClampedSize(size=self._size + other, min_size=self._min_size, max_size=self._max_size)
    ...
    def get(self, min_value: float = 0, max_value: Optional[float] = None, other_value: float = 0) -> float:
        min_size = (
            self._min_size.get(min_value, max_value, other_value) if self._min_size is not None else -math.inf
        )
        max_size = (
    ...
    def copy(self) -> "ClampedSize":
        return ClampedSize(self._size.copy(), self._min_size.copy(), self._max_size.copy())
