...
class Icon(Element):
    def __init__(self, name, size: Union[tuple[float, float], None] = None,
        ...
        self.set_icon(name)
        super().__init__(*self._surface.get_size() if size is None else size, selectable=False)
    ...
    def set_icon(self, name, color: Union[str, tuple[int, int, int], pygame.Color, None] = None, animation=None):
        try:
            if animation:
                old_element = Icon(self._icon, size=(self.width, self.height), color=self.col,
                                             style=self.style)
