...
class SelectableHStack(SelectableStack, HStack):
    def __init__(self,
                 # Stack parameters
    ...
    def _update_elements(self):
        ...
        if self.row_selected is None:
            if self._elements:
                ...
                self.sel_size_anim.val = self.row_selected.get_width(0)

    def update_rect(self, pos, max_size, root: View,
                    _ignore_fill_width: bool = False, _ignore_fill_height: bool = False):
        HStack.update_rect(self, pos, max_size, root, _ignore_fill_width, _ignore_fill_height)
        if self._defer_selection and self.row_selected is not None:
            self.sel_pos_anim.val = self.row_selected.rect.x - self.rect.x
            self.sel_size_anim.val = self.row_selected.rect.w
        ...
        if material:
            self.highlight_material_controller.render(self, surface,
                            (rect.x - surface.get_abs_offset()[0] +
                             ...
                             rect.y - surface.get_abs_offset()[1]),
                            (self.sel_size_anim.val + sum(self.selection_overdraw), rect.h),
                            alpha)
