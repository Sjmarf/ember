import pygame
from typing import Union, Literal, Optional, Sequence, NoReturn
...
from ember.ui.list import List
...
class VList(List, VStack):
    ...
    def __repr__(self):
        return f"<VList({len(self._elements)} elements)>"
    ...
    def _update_elements(self, animation: Optional[Transition] = None,
        ...
        if self.selected is None:
            if self._elements:
                ...
                self.sel_size_anim.val = self.selected.get_abs_height(0)

    def _load_element(self, element: Union[str, Element]) -> Element:
        element = load_element(element, text_height=self._list_style.text_height)
    ...
    def _update_rect_chain_down(self, surface: pygame.Surface, pos: tuple[float, float], max_size: tuple[float, float],
        ...
        VStack._update_rect_chain_down(self, surface, pos, max_size, root, _ignore_fill_width, _ignore_fill_height)
        if self._defer_selection and self.selected is not None:
            self.sel_pos_anim.val = self.selected.rect.y - self.rect.y
            self.sel_size_anim.val = self.selected.rect.h
                ...
                for element in self._elements:
                    if element.rect.collidepoint((element.rect.x, _c.mouse_pos[1])):
                        if not (hasattr(element, 'disabled') and element.disabled):
                            ...
                            if root.element_focused in self._elements:
